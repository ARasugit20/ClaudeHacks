...
function NotificationsDropdown({ onClose }) {
  ...
  return (
    <div ref={ref} style={{ position: "absolute", top: "calc(100% + 10px)", right: 0, zIndex: 9000, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 14, boxShadow: "0 16px 48px rgba(0,0,0,0.25)", width: 360, overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", borderBottom: "1px solid var(--border)" }}>
        <p style={{ fontSize: 14, fontWeight: 700, color: "var(--text)" }}>Notifications {unread > 0 && <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 7px", borderRadius: 999, background: "#ef4444", color: "white", marginLeft: 6 }}>{unread}</span>}</p>
      ...
      <div style={{ maxHeight: 400, overflowY: "auto" }}>
        {notifications.map(n => (
          <Link key={n.id} href={n.href} onClick={onClose} style={{ display: "flex", gap: 10, padding: "12px 16px", textDecoration: "none", borderBottom: "1px solid var(--border)", background: n.read ? "transparent" : "rgba(37,99,235,0.05)", transition: "background 0.15s" }}
...
function ProfileDropdown({ onClose }) {
  ...
  return (
    <div ref={ref} style={{ position: "absolute", top: "calc(100% + 10px)", right: 0, zIndex: 9000, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 14, boxShadow: "0 16px 48px rgba(0,0,0,0.25)", width: 200, overflow: "hidden" }}>
      {[
    ...
    <>
      <nav className="nav">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          {/* Shortcuts */}
          <button onClick={() => setShowShortcuts(v => !v)} title="Keyboard shortcuts (?)" style={iconBtnStyle}
          ...
          <div style={{ position: "relative" }}>
            <button onClick={() => { setShowNotifs(v => !v); setShowProfile(false); }} style={iconBtnStyle}
              onMouseEnter={e => { e.currentTarget.style.borderColor = "#2563eb"; e.currentTarget.style.color = "#2563eb"; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = "var(--border)"; e.currentTarget.style.color = "var(--muted)"; }}>
          ...
          {/* Profile */}
          <div style={{ position: "relative" }}>
            <button onClick={() => { setShowProfile(v => !v); setShowNotifs(false); }} style={{ ...iconBtnStyle, background: "linear-gradient(135deg,#2563eb22,#7c3aed22)", borderColor: "rgba(37,99,235,0.3)" }}
